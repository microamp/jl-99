...
function lsort(colls)
    sort(colls, by=length)
end

colls = Vector{}[[:a, :b, :c], [:d, :e], [:f, :g, :h], [:d, :e], [:i, :j, :k, :l], [:m, :n], [:o]]
sorted_by_length = lsort(colls)
@assert sorted_by_length == Vector{}[[:o], [:d, :e], [:d, :e], [:m, :n], [:a, :b, :c], [:f, :g, :h], [:i, :j, :k, :l]]

println("Tests passed: JL-28.jl")
